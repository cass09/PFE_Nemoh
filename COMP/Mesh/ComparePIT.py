...
def Trace_Fex(files, files_ph, num_dof, num_beta, titre, distance, NOMS, mesh):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"IT_{mesh}_Fex_abs_{distance}_b{num_dof}.png", dpi=300)
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"IT_{mesh}_Fex_phase_{distance}_b{num_dof}.png", dpi=300)
    # plt.show()
    return
    ...
    for k in range(len(files)):
        ...
        for j, dof in enumerate(dof_vect):
            ...
            for i in range(len(frequencies)):
                ...
                # Stocker pour le calcul du RMSE
            ...
            # rmse = np.sqrt(mse)
            # # Normalisation par la moyenne des valeurs de référence
            # mean_ref = np.mean(ref_values)
            # nrmse = rmse / mean_ref
            # # Goodness of Fit basé sur NRMSE
            # gof = (1 - nrmse) * 100
            # # On garde aussi le "pire" NRMSE sur les fréquences si tu veux un min
            # min_gof = 100 * np.min(1 -(np.sqrt(np.array(diffs_squared)) / np.array(ref_values)))
            gof = np.mean(gof_local)
            min_gof = np.min(gof_local)
            mean_CoF[j, k] = gof
            min_CoF[j, k] = min_gof
            if gof < 90:
                print(f"dof={dof} - mesh={NOMS[k]} - mean GoF={gof:.2f}% - min GoF={min_gof:.2f}%")
            if gof < 0:
                print(f" !!!!!! dof={dof} - mesh={NOMS[k]} - mean GoF={gof:.2f}% - - min GoF={min_gof:.2f}%")
...
def Calcul_GOF_Fex(REF, files, Nom_data, dof_vect, NOMS, mesh):
    ...
    for k in range(len(files)) : 
        ...
        for j, dof in enumerate(dof_vect):
            ...
            min_gof_abs = np.min(gof_local_abs)
            min_CoF[j, k] = min_gof_abs
...
CPU=False
if geo=="barge" :
    folder="OuterCylinder/PIT3_barge_mesh"
    mesh="barge"    
    file=[f"{folder}1", f"{folder}2", f"{folder}3", f"{folder}4", f"{folder}5", f"{folder}6", f"{folder}7", f"{folder}8", f"{folder}9"]
    Nmesh=8
    NOMS=["Nmesh=8", "Nmesh=32", "Nmesh=50", "Nmesh=200", "Nmesh=450", "Nmesh=630", "Nmesh=800", "Nmesh=1250"]
    # NOMS=["Nmesh=8", "Nmesh=32", "Nmesh=50", "Nmesh=200", "Nmesh=450", "Nmesh=630", "Nmesh=800", "Nmesh=1250", "Nmesh=1800"]
    CPU_data=[375.908936, 674.773438, 1341.02722, 1862.98352, 2914.78955, 3654.30127, 5382.15234, 9549.78027]
elif geo=="cylinder" :
    folder="OuterCylinder/PIT3_cylinder_mesh"
    mesh="cylinder"    
    file=[f"{folder}1", f"{folder}2", f"{folder}3", f"{folder}4", f"{folder}5", f"{folder}6", f"{folder}7", f"{folder}8"]
    Nmesh=8
    NOMS=["Nmesh=40", "Nmesh=160", "Nmesh=260", "Nmesh=360", "Nmesh=504", "Nmesh=640", "Nmesh=1000", "Nmesh=1500"]
...
titre=f"Convergence study on outer cylinder for {mesh} body mesh"
...
for i in range(Nmesh) :
    CMfiles.append(f"{chemin}{file[i]}/resultsIT/Global_Madd_{distance}.00.dat")
    CAfiles.append(f"{chemin}{file[i]}/resultsIT/Global_Crad_{distance}.00.dat")
    Fefiles.append(f"{chemin}{file[i]}/resultsIT/Global_Fe_abs_{distance}.00.dat")
    Fepfiles.append(f"{chemin}{file[i]}/resultsIT/Global_Fe_phase_{distance}.00.dat")


if GRAPHS :
    for ind, dof in enumerate([1, 3, 5, 7, 9, 11]) : 
