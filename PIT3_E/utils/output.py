...
def WriteData(WECArr, directionMB, farm, distance, results, results_h5, Motion, Method):
    ...
    if farm["Output_type"]=="DAT" : 
        fe_abs_file_path = os.path.join(results,  f"Global_{test}Fe_abs_{type}{beta_value}d{config_d}{distance:.2f}.dat")
        fe_phase_file_path = os.path.join(results,  f"Global_{test}Fe_phase_{type}{beta_value}d{config_d}{distance:.2f}.dat")
        ...
        #################################################################
        if ONEbeta : 
            madd_file_path = os.path.join(results, f"Global_{test}Madd_{type}{beta_value}d{config_d}{distance:.2f}.dat")
            crad_file_path = os.path.join(results,  f"Global_{test}Crad_{type}{beta_value}d{config_d}{distance:.2f}.dat")
        ...
        #################################################################
        if farm['RAO'] :
            RAO_file_path = os.path.join(Motion,  f"Global_{test}RAO_{type}{beta_value}d{config_d}{distance:.2f}.dat")
        ...
        if farm['Kochin']['number']>0 :
            ...
            for i, period in enumerate(w): 
                for dir in range(len(WECArr.KochinS[0, :, 0])):
                    Kochin_file_path = os.path.join(Motion,  f"Global_{test}Kochin_{type}{beta_value}d{config_d}{distance:.2f}_pb{ind_pb}.dat")
