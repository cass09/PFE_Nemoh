...
class Body(object):
    ...
    def Transfers(self, dirSet, dirDy, dirFP, Evanescent, BEM ='N', Tol=1e-6, check=True):
        ...
        if dirFP:
            (self.D, self.G, self.AR, self.order, self.truncorder) = transfers(self.depth, self.dir, self.period,
                                                                               (self.radius, self.fpazimuth, self.fpdepth),
                                                                               PhiS, PhiR, self.Fex, Tol, self.convention, 
                                                                               Evanescent, self.Nmodes_E)
        else:
            self.D = self.G = self.AR = self.order = self.truncorder = self.radius = self.fpazimuth = self.fpdepth = dirFP
