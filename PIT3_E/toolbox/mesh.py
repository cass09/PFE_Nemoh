...
class SurF(object) :
    ...
    def triangles(self) :
        ...
        for n, pan in enumerate(self.coord) :
            if np.allclose(pan[0],pan[1]) :
                tri.append(n)
                self.coord[n] = pan[(0,1,2,3),:]
            elif np.allclose(pan[0],pan[3]) :
                tri.append(n)
                self.coord[n] = pan[(3,0,1,2),:]
            elif np.allclose(pan[1],pan[2]) :
                tri.append(n)
                self.coord[n] = pan[(1,2,3,0),:]
            elif np.allclose(pan[2],pan[3]) :
                tri.append(n)
    ...
    def Show_Panels2D(self, axes=(0,1)):
        """
        """
        fig = plt.figure()
        ax = fig.add_subplot(111)
        for pan in self.coord:
            ax.plot(pan[:,axes[0]],pan[:,axes[1]],'r-')
            # connect with a red line the last node with the first one
            ax.plot(pan[(0,-1),axes[0]],pan[(0,-1),axes[1]],'r-')
    ...
    def refine_mesh(self, option=2, triangles=False):
        """
        triangles=False refines only quadrilaters
        triangles=True refines also triangles
        option = 1 splits into 2 triangles the quadrilaters
        option = 2 splits into 4 triangles the quadrilaters
        """
        self.NCA()
...
def mesher2D(Ps, Nv, lims=[False, False, False, False]):
    """
    Ps list() len=2 [P0, Pf]
    P0 shape (Np, 2) Initial points
    Pf shape (Np, 2) Final points
    Nv int discretization along (Pf-P0)
    lims list() len=4 [xmin, xmax, ymin, ymax]
    """
    
    # Mesh in between Ps[0] and Ps[1]
    Np = len(Ps[0])
    V = Ps[1]-Ps[0]
    for ax, shift, lim in zip([0, 0, 1, 1], [-1., 1., -1., 1.], lims):
        if type(lim) != bool:
            for i in range(2):
                cond = (Ps[i][:, ax]-lim)*shift > 0.
                Ps[i][cond] = (((lim-Ps[0][cond, ax])/V[cond, ax])*V[cond].T+Ps[0][cond].T).T
                V = Ps[1]-Ps[0]
    nodes = np.array([V*scale+Ps[0] for scale in np.linspace(0., 1., Nv)]).reshape((-1, 2))
    nodes = np.array([nodes[:, 0], nodes[:, 1], np.zeros(nodes[:,0].shape)]).T
    ind = np.arange(Np-1)
    conectivity = np.array([ind, ind+1, ind+1+Np, ind+Np]).T
    conectivity = np.array([conectivity+Np*i for i in range(Nv-1)], int).reshape((-1, 4))
    surface = SurF(nodes[conectivity])
    return surface
