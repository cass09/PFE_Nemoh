...
def transfers_sources(water_depth,
    ...
    dim=2*targ_order+1
    diffmat = np.zeros((len(periods), (dim)*(Nmodes_E+1), (dim)*(Nmodes_E+1)), dtype=complex)
    diffmat_w = np.zeros(((dim)*(Nmodes_E+1), (dim)*(Nmodes_E+1)), dtype=complex)
    frcmat = np.zeros((len(periods), (dim)*(Nmodes_E+1), fex.shape[-1]), dtype=complex)
    a_s_rad = np.zeros((len(periods), fex.shape[-1], (dim)*(Nmodes_E+1)), dtype=complex)
    a_s_rad_w = np.zeros((fex.shape[-1],(dim)*(Nmodes_E+1)), dtype=complex)
    a_s_scat = np.zeros((len(periods), source_scat.shape[1], (dim)*(Nmodes_E+1)), dtype=complex)
    # dirs, modes = np.meshgrid(directions, range(-targ_order, targ_order+1),
    #                           indexing='ij', sparse=True)
    g=9.81
    print(source_rad.shape)
    for ind, per in enumerate(periods):
        ...
        a_s_rad_w[:,0:dim]=1j/2*C0*np.cosh(k0*water_depth)*int_rad
        diffmat_w[:,0:dim] = 1j/2*C0*np.cosh(k0*water_depth)*int_scat
        if Nmodes_E>0 : 
            int_rad_E = integral_sources_E(wave_cond, BodyMesh, source_rad[ind], Nmodes_E, targ_order)
            int_scat_E = integral_sources_E(wave_cond, BodyMesh, source_scat[ind], Nmodes_E, targ_order)
            a_s_rad_w[:,dim:]=-1/np.pi*int_rad_E
        ...
        for e in range(Nmodes_E+1) :
            for m in range(-targ_order, targ_order+1):
                # print(e, m, e*dim+m+targ_order)
                frcmat[ind][e*dim+m+targ_order,:]=-1j*fex[ind][e*dim+m+targ_order,:]*g/(2*np.pi/per)
                # a_s_rad[ind][:, m+targ_order]=(-1)**(-m)*(2.*np.pi/per)**2/g*np.conj(a_s_rad_w[:,-m+targ_order])
                a_s_rad[ind][:, e*dim+m+targ_order]=1j*(2*np.pi/per)/g*(a_s_rad_w[:,e*dim+m+targ_order])
                # for q in range(-targ_order, targ_order+1):
                #     diffmat[ind][e*dim+m+targ_order, e*dim+q+targ_order]=diffmat_w[e*dim+m+targ_order,e*dim+q+targ_order]
...
def integral_sources(wave_cond, BodyMesh, source, trunc_ord):
    ...
    ------
    A : np.ndarray
        Matrice complexe de forme (len(sigma_all), 2M+1)
    """
    centers=BodyMesh[0]
    areas=BodyMesh[1]
    r = centers[:, 0]
...
def integral_sources_E(wave_cond, BodyMesh, source, L, trunc_ord):
    # sigma_all, centers, areas, k, k0, d, M, p_values=None):
    """
    Calcule une matrice de projection :
        A_{mp} = ∫_S σ_p(r,θ,z) * I_m(kl r) * cos(kl(z + d)) * exp(-imθ) dS
    ...
    Np = len(source[:,0])
    A = np.zeros((Np, (2 * trunc_ord + 1)*L), dtype=complex)
    g=9.81
    K=w**2/g
    for l in range(L) : 
        ...
        for mi, mode in enumerate(range(-trunc_ord, trunc_ord + 1)):
            Im_kr = iv(mode, kl * r)
            exp_m_theta = np.exp(-1j * mode * theta)
            col_index = l * (2 * trunc_ord + 1) + mi 
            for di in range(Np):
                sigma_p = source[di]
                integrand = sigma_p * Im_kr * cosz * exp_m_theta
                A[di, col_index] = Cn*np.sum(integrand * areas)
