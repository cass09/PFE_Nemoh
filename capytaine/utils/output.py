...
def WriteData(WECArr, directionMB, farm, distance, results, results_h5, Motion, Method):
    ...
    if len(directionMB)==1 and directionMB[0]!=0.00 :
        beta_value=f"beta_{directionMB[0]*180/np.pi}_"
        ONEbeta=False
    elif len(directionMB)>1 :
        beta_value=f"multi_beta_{len(directionMB)}_"
        ONEbeta=False
    else :
        beta_value=""
        ONEbeta=True
    ...
    if farm["Output_type"]=="DAT" : 
        fe_abs_file_path = os.path.join(results,  f"CapytaineIT_{test}Fe_abs_{type}{beta_value}d{config_d}{distance:.2f}.dat")
        fe_phase_file_path = os.path.join(results,  f"CapytaineIT_{test}Fe_phase_{type}{beta_value}d{config_d}{distance:.2f}.dat")
        
        #################################################################
        #################### Excitation Forces ##########################
        #################################################################
        with open(fe_abs_file_path, "w") as fe_file, open(fe_phase_file_path, "w") as fe_phase_file :
        ...
        #################################################################
        if ONEbeta : 
            madd_file_path = os.path.join(results, f"CapytaineIT_{test}Madd_{type}{beta_value}d{config_d}{distance:.2f}.dat")
            crad_file_path = os.path.join(results,  f"CapytaineIT_{test}Crad_{type}{beta_value}d{config_d}{distance:.2f}.dat")
            with open(madd_file_path, "w") as madd_file, open(crad_file_path, "w") as crad_file:
        ...
        #################################################################
        if farm['RAO'] :
            RAO_file_path = os.path.join(Motion,  f"CapytaineIT_{test}RAO_{type}{beta_value}d{config_d}{distance:.2f}.dat")
        ...
        if farm['Kochin']['number']>0 :
            ...
            for i, period in enumerate(w): 
                for dir in range(len(WECArr.KochinS[0, :, 0])):
                    Kochin_file_path = os.path.join(Motion,  f"CapytaineIT_{test}Kochin_{type}{beta_value}d{config_d}{distance:.2f}_pb{ind_pb}.dat")
        ...
        if farm['Free_surface']['Nx'] > 0 :
            ...
            Lx=farm['Free_surface']['Lx']
            Ly=farm['Free_surface']['Ly']
            x = np.linspace(-Lx/2, Lx/2, Nx)
            y = np.linspace(-Ly/2, Ly/2, Ny)
            for ind, period in enumerate(w): 
                for dir in range(len(WECArr.ETA_S[0, :, 0, 0])):
                    FS_file_path = os.path.join(Motion,  f"CapytaineIT_{test}FS_{type}{beta_value}d{config_d}{distance:.2f}_pb{ind_pb:05d}.dat")
                    ...
                    with open(FS_file_path, "w") as FS_file:
                        for i in range(Nx):  
                            for j in range(Ny):  
                                FS_file.write(f"     {x[i]:.6e}  ")   
                                FS_file.write(f"     {y[j]:.6e}  ")       
                                FS_file.write(f" {np.abs(WECArr.ETA_R[ind, dof, i,j]):.6e}  ")      
                                FS_file.write(f" {cmath.phase(WECArr.ETA_R[ind, dof, i,j]):.6e}  ")   
                                FS_file.write(f" {(WECArr.ETA_R[ind, dof, i,j]).real:.6e}  ")   
                                FS_file.write(f" {(WECArr.ETA_R[ind, dof, i,j]).imag:.6e}  ")   
                                FS_file.write("\n")
                    ind_pb=ind_pb+1
            ...
            for ind, period in enumerate(w): 
                for dir in range(len(WECArr.ETA[0, :, 0, 0])):
                    ETA_file_path = os.path.join(Motion,  f"CapytaineIT_{test}ETA_{type}{beta_value}d{config_d}{distance:.2f}_pb{ind_pb:05d}.dat")
        ...
        h5f.create_dataset(f'radiation_{type}_d{config_d}{distance}', data=FR)
        h5f.create_dataset(f'excitation_{type}_d{config_d}{distance}', data=WECArr.Fex)
